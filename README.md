...
接口`/sys/user/token`的返回值中`authorities`字段成员的`authority`字段值即为`权限字符串`

1. 如果它是一个**URI**，持有该权限即代表有访问该路径的接口的权限（**且通常不会对请求的参数进行限制**）。 如：持有`/sys/permission/page`即表示持有分页查询权限的权限。
2. 如果它是一个`ROLE_`开头的字符串，代表持有一个角色，字符串剩下的部分是角色名称
...
- 个人中心页需要判断进入的是当前账号的还是他人的，查询和修改使用的接口有所不同(是否有'admin')，进入他人个人中心需要对应查询权限
- 需要展示的信息
    1. 当前账号：自己的个人信息(昵称、电话、生日)，自己的认证/授权信息(id，用户名，持有的角色，持有的权限，注册时间)
    2. 他人账号：该用户的个人信息，该用户的认证/授权信息，**均需要对应权限**
- 界面提供功能
    1. 当前账号：修改密码、登出、修改个人信息
    1. 他人账号：添加、删除该用户持有的角色，修改过期时间（使用配置接口），**均需要对应权限**，另需要`分页查询角色`权限
